...
const ComposeMail = () => {
  ...
  const SendHandler = async (event) => {
    ...
    const mailData = {
     ...
     try {
      ...
      const response = await axios.post(
        `https://react-app-ff034-default-rtdb.firebaseio.com/${toFormattedEmail}inbox.json`,mailData);
    ...
    try{
     const response = await axios.post(
        `https://react-app-ff034-default-rtdb.firebaseio.com/${newMAil}sentbox.json`,{
          to:toFormattedEmail ,
      subject: enteredSubjectref,
      body: bodyText,
        });
